...
root = Path(r"C:\Users\uugur\OneDrive\Desktop\animations_ncs_emg\qa_nonphys_69")
...
for sheet_no in range((len(files) + cols * rows - 1) // (cols * rows)):
    ...
    for i, file in enumerate(subset):
        ...
        draw.text((x + 8, y + thumb_h + 6), label, fill="#e9f4f6", font=font)
    sheet.save(root / f"contact-sheet-{sheet_no + 1:02d}.jpg", quality=90)
