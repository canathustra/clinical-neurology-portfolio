...
root=Path(r"C:\Users\uugur\OneDrive\Desktop\animations_ncs_emg\qa_scale_filter_fix")
files=sorted(root.glob("*.png"))
font=ImageFont.load_default(size=14)
cols,rows=2,3
thumb_w,thumb_h,label_h=800,450,26
for page in range((len(files)+cols*rows-1)//(cols*rows)):
    ...
    for i,file in enumerate(subset):
        ...
        draw.text((x+8,y+thumb_h+5),file.stem,fill="#e9f4f6",font=font)
